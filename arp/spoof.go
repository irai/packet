...
func (h *Handler) spoofLoop(addr packet.Addr) {
	...
	for {
		...
		if !hunting || h.closed {
			...
			if err := h.announce(addr.MAC, h.session.NICInfo.RouterMAC, h.session.NICInfo.RouterIP4.IP, EthernetBroadcast); err != nil {
...
func (h *Handler) forceSpoof(addr packet.Addr) error {
	...
	err := h.announce(addr.MAC, h.session.NICInfo.HostMAC, h.session.NICInfo.RouterIP4.IP, EthernetBroadcast)
	if err != nil {
		log.Printf("arp error send announcement packet %s: %s", addr, err)
		return err
	}
