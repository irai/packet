...
func TestHandler_newHostMany(t *testing.T) {
	...
	tests := []TestEvent{}
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC1}, "mac1", 1, 1)...)
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC2}, "mac2", 1, 1)...)
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC3}, "mac3", 1, 1)...)
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC4}, "mac4", 1, 1)...)
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC5}, "mac5", 1, 1)...)
...
func TestHandler_sameHostMany(t *testing.T) {
	...
	tests := []TestEvent{}
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC1}, "mac1", +1, 1)...)
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC1}, "mac1", +0, 0)...) // dhcp will reuse ip
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC1}, "mac1", +0, 0)...) // dhcp will reuse ip
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC1}, "mac1", +0, 0)...) // dhcp will reuse ip
	tests = append(tests, NewHostEvents(packet.Addr{MAC: MAC1}, "mac1", +0, 0)...) // dhcp will reuse ip
